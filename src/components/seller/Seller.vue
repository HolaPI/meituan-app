  ...
  <div class="seller" ref="sellerView">
    <div class="seller-wrapper">
        ...
        <div class="pics-wrapper" ref="picsView">
          <ul class="pics-list" v-if="seller.poi_env" ref="picsList">
            <li
              ref="picsItem"
              class="pics-item"
              v-for="(pic, index) in seller.poi_env.thumbnails_url_list"
              :key="index"
            >
              <img :src="pic">
            </li>
      ...
      <div class="tip-view">
        ...
        <div class="shipping-time">配送时间： {{seller.shipping_time}}</div>
      </div>
      <app-split></app-split>
      <div class="other-view">
        ...
        <div class="discounts-wrapper">
          <div class="discount-item" v-for="(discount, index) in seller.discounts2" :key="index">
            <div class="icon">
  ...
  created() {
    fetch("/mock/seller.json")
      ...
      .then(response => {
        ...
        this.$nextTick(() => {
          if (!this.scorllY) {
            this.scrollY = new BScroll(this.$refs.sellerView, {
              touchend: true
            });
          } else {
...
.seller .seller-wrapper .tip-view {
  padding-left: 15px;
}
.seller .seller-wrapper .tip-view .delivery-wrapper {
...
.seller .seller-wrapper .tip-view .shipping-time {
...
.seller .seller-wrapper .other-view {
  padding-left: 15px;
}
...
.seller .seller-wrapper .other-view .server-wrapper .poi-server {
  display: inline-block;
  margin-left: 17px;
}
.seller .seller-wrapper .other-view .server-wrapper .poi-server img {
  width: 15px;
  height: 15px;
  margin-right: 6px;
  vertical-align: middle;
}
.seller .seller-wrapper .other-view .discounts-wrapper {
  padding: 17px 24px 19px 0;
}
.seller .seller-wrapper .other-view .discounts-wrapper .discount-item {
  display: flex;
}
.seller .seller-wrapper .other-view .discounts-wrapper .discount-item .icon {
  flex: 0 0 25px;
}
.seller
  .seller-wrapper
...
.seller .seller-wrapper .other-view .discounts-wrapper .discount-item .text {
  flex: 1;
  font-size: 14px;
}
</style>
